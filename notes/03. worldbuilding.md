...
### Small scripts for consistency & ease of use on a cloud machine

CARLA's Tutorial docs recommend starting with spectator navigation, but as far as I can tell, that's a different mental model for runpod (more of a remote desktop). 

Using script-based approaches is the more direct path for these tests, especially as it helps break out some key abstractions.
...
I also checked out the native `world_snapshot` methods, but the output object seems to serve a different purpose - a 'still image' of the actors and their positions, not including environment conditions.
...
These tests use synchronous mode with a fixed delta of 0.1 seconds. This configuration enables deterministic world stepping.

Previous versions also used random initial conditions with a fixed seed, but I removed it once I found a good enough set of conditions for visualising the variations.

My understanding is that the sensor modeling and Unreal-based rendering pipeline may still introduce small frame-level variations depending on hardware and GPU execution. 