...
## Setup (RunPod):
- RTX 4090 → ideal for Unreal + multiple sensors
- 20GB Container disk (temporary storage)
- 120GB Volume Disk (persistent storage), for CARLA and Model weights

- ~$0.61/hr (active), ~0.03 /hr (paused) → reasonable for focused sessions.
...
One issue is that RunPod pods are semi-ephemeral: you can lose the underlying GPU day-to-day. When RunPod reallocates the GPU, it migrates workspace data but not optional data—so CARLA (e.g. under `/opt/carla`) is gone and you need to re-set up the pod and re-download CARLA.

I added [infra/setup_runpod_carla.sh](../infra/setup_runpod_carla.sh) so a single command does everything:
- Updates local SSH config and ensures the `carla` user and keys on the pod
- Installs the GitHub deploy key for `carla` and updates the repo (git pull/clone)
- **Installs CARLA on the pod if it’s not already there** (e.g. after a migration)
- **Starts CARLA** so you’re ready to develop

Usage: call the script with RunPod’s SSH connection string (from "Connect to your Pod using SSH over a direct TCP connection"). By default it will install CARLA if missing and start it. Use `--no-install` only when CARLA is already installed and you just want to refresh setup and start CARLA.
